...
public abstract class JobStoreSupport implements JobStore, Constants {
        ...
        do {
            ...
            try {
                ...
                for(TriggerKey triggerKey: keys) {
                    ...
                    //todo 这里利用乐观锁机制，先wait——》acquired，保证只有一个实例能成功更新
                    ...
                    //todo 由于只有一个实例能成功更新trigger状态，那么也只有一个实例能成功插入fired_trigger表中数据,
                    //两个在同一个事务当中.后续触发的时候也只有一个实例可以触发调用
                    //这也有个问题，如何保证实例重启还能恢复任务或者释放任务，因为这里已经把这个任务跟实例绑定（任务的状态和触发任务的实例）
    ...
    protected TriggerFiredBundle triggerFired(Connection conn,
        ...
        try { // if trigger was deleted, state will be STATE_DELETED
        	...
        	//todo 这里如何保证后面更新trigger状态是正确的？没有找到锁的位置
        	//todo 有没有可能两个进程走到这里时，都发现状态是 STATE_ACQUIRED ，然后到了后面更新语句，变成 STATE_EXECUTING 
